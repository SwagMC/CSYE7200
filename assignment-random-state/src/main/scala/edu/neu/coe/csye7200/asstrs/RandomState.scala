...
trait RandomState[T] {
  ...
  // 10 points
  def flatMap[U](f: T=>RandomState[U]): RandomState[U] = f(get) // TO BE IMPLEMENTED done
  ...
  // 12 points
  def toStream: Stream[T] = Stream.cons(this.get, this.next.toStream) // TO BE IMPLEMENTED done
}
...
case class JavaRandomState[T](n: Long, g: Long=>T) extends RandomState[T] {
  ...
  // 7 points
  val r = new Random(n)
  def next: RandomState[T] = JavaRandomState(r.nextLong(), g) // TO BE IMPLEMENTED done
  // Hint: Think of the input and output.
  // 5 points
  def get: T = g(n) // TO BE IMPLEMENTED done
  // Hint: This one need function composition.
  // 13 points
  def map[U](f: (T) => U): RandomState[U] = JavaRandomState[U](n, n => f(g(n))) // TO BE IMPLEMENTED done
}
...
object RandomState {
  ...
  // 4 points
  val longToDouble: Long=>Double = _.toDouble / Long.MaxValue // TO BE IMPLEMENTED done
